...
async function getList(order) {
	const {
		data: { total, records },
	} = await api.get("/web/genera/page", { ...pagination, ...search.value, ...order })
	pagination.total = parseInt(total)
	tableData.value = records
}

const down = async () => {
	await api.down("/web/genera/exportExcel", { ...pagination, ...search.value })
}
	...
	<div class="content">
		...
		<div class="my-table">
			<el-table :data="tableData" border stripe size="default" @sort-change="sortChange">
				...
				<el-table-column sortable="custom" show-overflow-tooltip prop="generaName" label="Genera name" minWidth="200">
					<template #default="scope">
						<router-link :to="{ path: '/gene/index', query: { geneName: scope.row.generaName } }" class="table-link">{{ scope.row.generaName }}</router-link>
					</template>
				</el-table-column>
