...
async function getList(order) {
	const {
		data: { total, records },
	} = await api.get("/web/seMatsJcec/page", { ...pagination, ...search.value, dataAccessId, file })
...
const down = async () => {
	await api.down("/web/seMatsJcec/exportExcel", { ...pagination, ...search.value, dataAccessId, file })
}
...
<template>
	<div class="sub_content">
		<Search :search="search" :fn="getList" :down="down" title="sE"/>
		<div class="my-table">
			<el-table :data="tableData" border stripe size="default" @sort-change="sortChange">
				...
				<el-table-column sortable="custom" show-overflow-tooltip prop="exonStart0Base" label="exonStart_0base" min-width="200" />
				<el-table-column sortable="custom" show-overflow-tooltip prop="exonEnd" label="exonEnd" min-width="140" />
