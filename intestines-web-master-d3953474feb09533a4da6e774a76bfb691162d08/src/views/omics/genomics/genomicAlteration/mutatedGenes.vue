...
async function getList(order) {
	const {
		data: { total, records },
	} = await api.get("/web/mutatedGenes/page", { ...pagination, ...search.value, ...order })
	pagination.total = parseInt(total)
	tableData.value = records
}

const down = async () => {
	await api.down("/web/mutatedGenes/exportExcel", { ...pagination,...search.value })
}
...
<template>
	<div class="subContent">
		<Search :search="search" :fn="getList" :down="down" title="mutatedGenes"/>
		<div class="my-table">
				...
				<el-table-column sortable="custom" show-overflow-tooltip prop="freq" label="Freq" min-width="100">
					<template #header>
						...
						<el-popover :width="400">
							...
							<template #default>Percentage of samples with one or more mutations</template>
						</el-popover>
					</template>
					<template #default="scope">
