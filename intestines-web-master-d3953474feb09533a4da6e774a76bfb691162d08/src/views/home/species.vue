...
async function getList(order) {
	const {
		data: { total, records },
	} = await api.get("/web/homeSpecies/page", { ...pagination, ...search.value, ...order })
	pagination.total = parseInt(total)
	tableData.value = records
}

const down = async () => {
	await api.down("/web/homeSpecies/exportExcel", { ...pagination,...search.value })
}
		...
		<div class="my-table">
			<el-table :data="tableData" border stripe size="default" @sort-change="sortChange">
				...
				<el-table-column sortable="custom" show-overflow-tooltip prop="source" label="Source" minWidth="110">
					<template #default="scope">
						<a class="table-link" :href="scope.row.link" target="_blank">{{ scope.row.source }}</a>
					</template>
				</el-table-column>
