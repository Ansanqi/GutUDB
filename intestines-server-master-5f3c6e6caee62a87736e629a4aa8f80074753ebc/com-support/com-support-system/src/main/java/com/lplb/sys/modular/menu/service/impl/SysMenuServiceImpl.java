 ...
 */
package com.lplb.sys.modular.menu.service.impl;
...
import com.lplb.sys.core.cache.ResourceCache;
import com.lplb.sys.core.enums.MenuTypeEnum;
import com.lplb.sys.modular.menu.entity.SysMenu;
import com.lplb.sys.modular.menu.enums.SysMenuExceptionEnum;
import com.lplb.sys.modular.menu.mapper.SysMenuMapper;
...
import com.lplb.sys.modular.role.entity.SysRoleMenu;
import com.lplb.sys.modular.role.service.SysRoleMenuService;
import com.lplb.sys.modular.role.service.impl.SysRoleMenuServiceImpl;
...
public class SysMenuServiceImpl extends ServiceImpl<SysMenuMapper, SysMenu> implements SysMenuService {
    ...
    public List<String> getLoginPermissions(Long userId) {
        ...
        return CollUtil.newArrayList(permissions);
    }
...
//        }
    ...
    private List<LoginMenuTreeNode> convertSysMenuToLoginMenu(List<SysMenu> sysMenuList) {
        ...
        sysMenuList.forEach(sysMenu -> {
            ...
            loginMenuTreeNode.setMenuId(sysMenu.getId());
            ...
            loginMenuTreeNode.setSortNumber(sysMenu.getSort());
            loginMenuTreeNode.setMenuType(sysMenu.getType());
            loginMenuTreeNode.setTenantId(2);
            loginMenuTreeNode.setIcon(sysMenu.getIcon());
            loginMenuTreeNode.setTarget("_self");
            loginMenuTreeNode.setAuthority(sysMenu.getAuthority());
...
//            mateItem.setBslDesc(sysMenu.getBslDesc());
//            mateItem.setHideChildren("true".equals(sysMenu.getHideChildren()));
//            //是否可见
...
//            loginMenuTreeNode.setMeta(mateItem);
    ...
    public List<MenuBaseTreeNode> treeForGrant(SysMenuParam sysMenuParam) {
        ...
        if (LoginContextHolder.me().isSuperAdmin()) {
            queryWrapper.eq(SysMenu::getStatus, CommonStatusEnum.ENABLE.getCode());
            queryWrapper.eq(SysMenu::getWeight,"2");
        } else {
    ...
    @Override
    public List<String> getLoginFunctions(Long userId) {
        Set<String> functions = CollectionUtil.newHashSet();
        List<Long> roleIdList = sysUserRoleService.getUserRoleIdList(userId);
        if (ObjectUtil.isNotEmpty(roleIdList)) {
            List<Long> menuIdList = sysRoleMenuService.getRoleMenuIdList(roleIdList);
            if (ObjectUtil.isNotEmpty(menuIdList)) {
                ...
                this.list(queryWrapper).forEach(sysMenu -> functions.add(sysMenu.getFunctionFlag()));
            }
        }
        return CollectionUtil.newArrayList(functions);
    }

    @Override
    public List<Map<String, Object>> getLoginUserMenu(Long userId) {
        SysUser sysUser = sysUserService.getById(userId);
        List<String> roleIds = LoginContextHolder.me().getLoginUserRoleIds();
        if (ObjectUtil.isEmpty(sysUser)||roleIds.size()==0) {
            return null;
        }
        List<String> userMenusIds = this.getBaseMapper().getRoleMenuIds(roleIds);
        if(userMenusIds.size()==0){
            return new ArrayList<>();
        }
        List<Map<String,Object>> menus = this.getBaseMapper().getUserTopMenus(userMenusIds);
        return getSubMenu(menus,userMenusIds);
    }

    public List<Map<String, Object>> getSubMenu(List<Map<String, Object>> menus,List<String> userMenusIds) {
        menus.forEach(menu -> {
            if(!menu.containsKey("path")){
                menu.put("path",null);
            }
            if(!menu.containsKey("component")){
                menu.put("component",null);
            }
            menu.put("uid",null);
            List<Map<String, Object>> subMenus = this.getBaseMapper().getMenusByParentIdAndRole(menu.get("menuId").toString(),userMenusIds);
            if (subMenus.size() > 0) {
                menu.put("children", getSubMenu(subMenus,userMenusIds));
            }
        });
    ...
    @Override
    public List<Map<String, Object>> getMenuPage() {
        return this.getBaseMapper().getMenuPage();
    }

    @Override
    public void saveMenu(SysMenuParam sysMenuParam) {
        boolean isCreate = ObjectUtil.isEmpty(sysMenuParam.getMenuId());
        SysMenu sysMenu = new SysMenu();

        sysMenu.setSort(sysMenuParam.getSortNumber());
        sysMenu.setIcon(sysMenuParam.getIcon());
        sysMenu.setComponent(sysMenuParam.getComponent());
        sysMenu.setRouter(sysMenuParam.getPath());
        sysMenu.setName(sysMenuParam.getTitle());
        sysMenu.setType(sysMenuParam.getMenuType());
        sysMenu.setPid(sysMenuParam.getParentId());
        sysMenu.setOpenType(sysMenuParam.getOpenType());
        sysMenu.setVisible(sysMenuParam.getVisible());
//        sysMenu.setAuthority(sysMenuParam.getAuthority());
        sysMenu.setPermission(sysMenuParam.getPermission());
        sysMenu.setApplication("system");
        if (ObjectUtil.isNotEmpty(sysMenuParam.getMenuId())) {
            sysMenu.setId(sysMenuParam.getMenuId());
        }
        this.saveOrUpdate(sysMenu);
//        if (isCreate) {
//            SysRoleMenu roleMenu = new SysRoleMenu();
//            roleMenu.setRoleId(1412966870876696578L);
...
//        }
    ...
    public List<Map<String, Object>> getRoleMenu(Long roleId) {
        return this.getBaseMapper().getRoleMenus(roleId);
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public void saveRoleMenu(Long roleId, JSONArray menus) {
        List<SysRoleMenu> roleMenus = new ArrayList<>();
        this.getBaseMapper().removeRoleMenu(roleId);
        if(menus.size()>0){
            for (Object menu : menus) {
                SysRoleMenu roleMenu = new SysRoleMenu();
                roleMenu.setRoleId(roleId);
                roleMenu.setMenuId(Long.parseLong(menu.toString()));
                roleMenus.add(roleMenu);
            }
            sysRoleMenuService.saveOrUpdateBatch(roleMenus);
        }
    }
}
