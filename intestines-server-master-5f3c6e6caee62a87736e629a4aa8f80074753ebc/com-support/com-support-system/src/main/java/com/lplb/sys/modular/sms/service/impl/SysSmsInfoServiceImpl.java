 ...
 */
package com.lplb.sys.modular.sms.service.impl;
...
import com.lplb.core.pojo.sms.AliyunSmsConfigs;
import com.lplb.sys.modular.sms.entity.SysSms;
import com.lplb.sys.modular.sms.enums.SmsSendExceptionEnum;
import com.lplb.sys.modular.sms.enums.SmsSendStatusEnum;
import com.lplb.sys.modular.sms.enums.SmsVerifyEnum;
import com.lplb.sys.modular.sms.mapper.SysSmsMapper;
import com.lplb.sys.modular.sms.param.SysSmsInfoParam;
import com.lplb.sys.modular.sms.param.SysSmsSendParam;
import com.lplb.sys.modular.sms.param.SysSmsVerifyParam;
import com.lplb.sys.modular.sms.service.SysSmsInfoService;
...
public class SysSmsInfoServiceImpl extends ServiceImpl<SysSmsMapper, SysSms> implements SysSmsInfoService {
    ...
    private static final Logger logger = LoggerFactory.getLogger(SysSmsInfoServiceImpl.class);
    ...
    public Long saveSmsInfo(SysSmsSendParam sysSmsSendParam, String validateCode) {
        ...
        logger.info(">>> 发送短信，存储短信到数据库，数据为：" + JSON.toJSONString(sysSms));
    ...
    public SmsVerifyEnum validateSmsInfo(SysSmsVerifyParam sysSmsVerifyParam) {
        ...
        logger.info(">>> 验证短信Provider接口，查询到sms记录：" + JSON.toJSONString(sysSmsList));

        //如果找不到记录，提示验证失败
        if (ObjectUtil.isEmpty(sysSmsList)) {
            logger.info(">>> 验证短信Provider接口，找不到验证码记录，响应验证失败！");
            ...
            if (SmsSendStatusEnum.INVALID.getCode().equals(sysSms.getStatus())) {
                logger.info(">>> 验证短信Provider接口，短信状态是失效，响应验证失败！");
                return SmsVerifyEnum.ERROR;
            }

            //如果验证码和传过来的不一致
            if (!sysSmsVerifyParam.getCode().equals(sysSms.getValidateCode())) {
                logger.info(">>> 验证短信Provider接口，验证手机号和验证码不一致，响应验证失败！");
                return SmsVerifyEnum.ERROR;
            }
            ...
            if (ObjectUtil.isEmpty(invalidTime) || new Date().after(invalidTime)) {
                logger.info(">>> 验证短信Provider接口，验证码超时，响应验证失败！");
            ...
            logger.info(">>> 验证短信Provider接口，验证码验证成功！");
