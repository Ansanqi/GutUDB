 ...
 */
package com.lplb.sys.modular.role.service.impl;
...
import com.lplb.sys.modular.role.enums.SysRoleExceptionEnum;
import com.lplb.sys.modular.role.mapper.SysRoleMapper;
...
public class SysRoleServiceImpl extends ServiceImpl<SysRoleMapper, SysRole> implements SysRoleService {
    ...
    public PageResult<SysRole> page(SysRoleParam sysRoleParam) {
        ...
        queryWrapper.eq(SysRole::getType,"2");
        //根据排序升序排列，序号越小越在前
    ...
    @Override
    public List<SysRole> listRoles(SysRoleParam sysRoleParam) {
        LambdaQueryWrapper<SysRole> queryWrapper = new LambdaQueryWrapper<>();
        if (ObjectUtil.isNotNull(sysRoleParam)) {
            //根据名称模糊查询
            if (ObjectUtil.isNotEmpty(sysRoleParam.getName())) {
    ...
    public void delete(Long roleId) {
        ...
        //级联删除该角色对应的角色-数据范围关联信息Ω
        sysRoleDataScopeService.deleteRoleDataScopeListByRoleId(roleId);

        //级联删除该角色对应的用户-角色表关联信息
        sysUserRoleService.deleteUserRoleListByRoleId(roleId);

        //级联删除该角色对应的角色-菜单表关联信息
        sysRoleMenuService.deleteRoleMenuListByRoleId(roleId);
    }
    ...
    @Override
    public SysRole getRoleByRoleCode(String code) {
        LambdaQueryWrapper<SysRole> queryWrapper = new LambdaQueryWrapper<>();
