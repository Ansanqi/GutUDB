 ...
 */
package com.lplb.sys.modular.auth.factory;
...
import com.lplb.core.pojo.login.LoginEmpInfo;
import com.lplb.core.pojo.login.SysLoginUser;
import com.lplb.core.tenant.consts.TenantConstants;
...
import com.lplb.core.util.UaUtil;
import com.lplb.sys.modular.app.service.SysAppService;
...
public class LoginUserFactory {
    ...
    public static void fillLoginUserInfo(SysLoginUser sysLoginUser) {
        ...
        if (ObjectUtil.isNotNull(request)) {
            ...
            sysLoginUser.setAvatar(CommonConstant.DEFAULT_AVATAR);
            Long userId = sysLoginUser.getId();
            sysLoginUser.setSysSecretLevel("密级：机密");
            // 员工信息
            ...
            // 权限信息
            List<String> functions = sysMenuService.getLoginFunctions(userId);
            sysLoginUser.setFunctions(functions);
            ...
            //判断当前组织架构模式
            String systemOrgIndependent = ConstantContextHolder.getSystemOrgIndependent();
            sysLoginUser.setSysOrgType("false".equals(systemOrgIndependent)?"1":"0");

            // 如果根本没有应用信息，则没有菜单信息
            if (ObjectUtil.isEmpty(apps)) {
                sysLoginUser.setMenus(CollUtil.newArrayList());
            } else {
