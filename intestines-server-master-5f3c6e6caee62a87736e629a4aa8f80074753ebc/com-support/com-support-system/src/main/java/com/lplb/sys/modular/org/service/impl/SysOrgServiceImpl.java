 ...
 */
package com.lplb.sys.modular.org.service.impl;
...
import com.lplb.core.factory.TreeBuildFactory;
...
import com.lplb.sys.core.enums.DataScopeTypeEnum;
import com.lplb.sys.modular.emp.service.SysEmpExtOrgPosService;
...
import com.lplb.sys.modular.org.enums.SysOrgExceptionEnum;
import com.lplb.sys.modular.org.mapper.SysOrgMapper;
import com.lplb.sys.modular.org.param.SysOrgParam;
import com.lplb.sys.modular.org.service.SysOrgInfoService;
import com.lplb.sys.modular.org.service.SysOrgService;
import com.lplb.sys.modular.role.service.SysRoleDataScopeService;
...
public class SysOrgServiceImpl extends ServiceImpl<SysOrgMapper, SysOrg> implements SysOrgService {
    ...
    @Override
    public PageResult<Map<String,Object>> page(SysOrgParam sysOrgParam) {
        Page<Map<String,Object>> result = this.getBaseMapper().getOrgPage(PageFactory.defaultPage(),ObjectUtil.isNotEmpty(sysOrgParam)?sysOrgParam.getName():null);
...
//            }
//        }
//        queryWrapper.ne(SysOrg::getPid,0);
...
//        sysOrgInfo.setOrgUuid(sysOrg.getId());
//        sysOrgInfo.setAdvertiserId(sysOrgParam.getAdvertiserId());
//        sysOrgInfo.setOpenAppId(1L);
//        sysOrgInfoService.saveOrUpdate(sysOrgInfo);
    }
...
//        }
        ...
        LambdaQueryWrapper<SysOrgInfo> infoQuery = new LambdaQueryWrapper<>();
        infoQuery.eq(SysOrgInfo::getOrgUuid,sysOrg.getId());
        infoQuery.last("limit 1");
        SysOrgInfo sysOrgInfo = sysOrgInfoService.getOne(infoQuery);
        sysOrgInfo.setAdvertiserId(sysOrgParam.getAdvertiserId());
        sysOrgInfoService.saveOrUpdate(sysOrgInfo);
    }
    ...
    public List<AntdBaseTreeNode> tree(SysOrgParam sysOrgParam) {
        ...
        if (!superAdmin) {
            ...
            if (ObjectUtil.isEmpty(dataScope)) {
                ...
                Set<Long> dataScopeSet = CollectionUtil.newHashSet(dataScope);
                List<Dict> extOrgPos = sysEmpExtOrgPosService.getEmpExtOrgPosDictList(LoginContextHolder.me().getSysLoginUserId(), true);
                for (Dict ext : extOrgPos) {
                    Long extOrg = ext.getLong("orgId");
                    dataScopeSet.add(extOrg);
                }
                dataScope.forEach(orgId -> {
                    //此处获取所有的上级节点，放入set，用于构造完整树
                    List<Long> parentAndChildIdListWithSelf = this.getParentIdListById(orgId);
        ...
        //根据排序升序排列，序号越小越在前
        queryWrapper.orderByAsc(SysOrg::getSort);
        List<Long> existOrg = new ArrayList<>();
        this.list(queryWrapper).forEach(sysOrg -> {
            fillTreeNode(treeNodeList, sysOrg, existOrg);
...
//            treeNodeList.add(orgTreeNode);
    ...
    public void fillTreeNode(List<AntdBaseTreeNode> nodes, SysOrg sysOrg, List<Long> existOrg) {
        if (!existOrg.contains(sysOrg.getId())) {
            AntdBaseTreeNode orgTreeNode = new AntdBaseTreeNode();
            orgTreeNode.setId(sysOrg.getId());
            orgTreeNode.setParentId(sysOrg.getPid());
            orgTreeNode.setTitle(sysOrg.getName());
            orgTreeNode.setValue(String.valueOf(sysOrg.getId()));
            orgTreeNode.setWeight(sysOrg.getSort());
            nodes.add(orgTreeNode);
            existOrg.add(sysOrg.getId());
            if (!sysOrg.getPid().equals(SystemCommonConstant.TOP_ORG_ID) && !sysOrg.getPid().equals(0L)) {
                SysOrg pOrg = this.getOrg(sysOrg.getPid());
                fillTreeNode(nodes, pOrg, existOrg);
            }
        }
    }
...
//        }
    ...
    private void fillPids(SysOrg sysOrg) {
        sysOrg.setPid(1410139203211223042L);
        sysOrg.setPids(SymbolConstant.LEFT_SQUARE_BRACKETS+"1410139203211223042"+SymbolConstant.RIGHT_SQUARE_BRACKETS);
...
//        }
    ...
    public List<String> getOrgNames() {
        return this.getBaseMapper().getAllOrgNames();
    }
    ...
    public List<String> getOrgCodeNames() {
        ...
        lambdaQueryWrapper.ne(SysOrg::getPid,"0");
        lambdaQueryWrapper.orderByAsc(SysOrg::getCreateTime);
        List<SysOrg> list = this.list(lambdaQueryWrapper);
        for (SysOrg sysOrg : list) {
            result.add(sysOrg.getName()+"("+sysOrg.getCode()+")");
        }
        return result;
    }
    ...
    public String getOrgByName(String orgName) {
        ...
        SysOrg org = this.getOne(queryWrapper);
        return ObjectUtil.isNotEmpty(org) ? org.getId().toString() : null;
    }

    @Override
    public SysOrg getOrgByNameAndCode(String str) {
        return this.getBaseMapper().getOrgByNameAndCode(str);
    }

    @Override
    public void downloadImportTpl(HttpServletResponse response) {
        Integer versions = DateTimeUtil.getNowStampUnix();
        String fileName = URLUtil.encode("【非密】软件配置管理系统部门信息导入模板") + versions + ".xls\"";
        String[] orgHeaders = {"*部门名称", "*部门编码"};
        Workbook workbook = WorkbookUtil.createBook(false);
        ExcelTemplateUtil.createExcelTemplate(workbook, orgHeaders, null, null,
                "部门信息", versions.toString(), 1,null,false);
        try {
            response.reset();
...
//            out.close();
    ...
    @Override
    public Object importOrg(Workbook workbook) {
        ...
        for (int i = firstRowNum + 2; i <= lastRowNum; i++) {
            ...
            SysOrg sysOrg = new SysOrg();
            //部门名称
            Cell orgNameCell = row.getCell(0);
            if (ExcelTemplateUtil.checkCellValue(orgNameCell)) {
                String orgName = orgNameCell.getStringCellValue();
                sysOrg.setName(orgName);
            } else {
                return ErrorResponseData.error("导入失败，行号："+(i+1)+",错误信息:部门名称不能为空");
            }
            Cell orgCodeCell = row.getCell(1);
            if (ExcelTemplateUtil.checkCellValue(orgCodeCell)) {
                String orgCode = orgCodeCell.getStringCellValue();
                if(existCode.contains(orgCode)){
                    return ErrorResponseData.error("导入失败，行号："+(i+1)+",错误信息:部门编号"+orgCode+"重复");
                }
                if(orgCodeExist(orgCode)){
                    return ErrorResponseData.error("导入失败，行号："+(i+1)+",错误信息:部门编号"+orgCode+"已存在");
                }
                sysOrg.setCode(orgCode);
                existCode.add(orgCode);
            } else {
                return ErrorResponseData.error("导入失败，行号："+(i+1)+",错误信息:部门名称不能为空");
            }
            sysOrg.setStatus(CommonStatusEnum.ENABLE.getCode());
            sysOrg.setPid(1410139203211223042L);
            sysOrg.setPids("[0],[1410139203211223042],");
            orgs.add(sysOrg);
        }
        if(orgs.size()>0){
            this.saveOrUpdateBatch(orgs);
        }else{
            return ErrorResponseData.error("导入失败，未检测到有效的部门数据");
        }
        return SuccessResponseData.success("成功导入"+orgs.size()+"条数据");
    }

    public boolean orgCodeExist(String orgCode){
        LambdaQueryWrapper<SysOrg> queryWrapper = new LambdaQueryWrapper<>();
    ...
    @Override
    public List<Map<String, Object>> selectList(Long orgId) {
        return this.getBaseMapper().getSelectList(orgId);
    }

    @Override
    public List<SysOrg> getOrgList() {
        return this.getBaseMapper().getAdOrgList();
    }
}
