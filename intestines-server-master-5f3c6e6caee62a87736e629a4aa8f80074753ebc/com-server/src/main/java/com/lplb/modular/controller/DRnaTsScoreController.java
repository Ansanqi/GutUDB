package com.lplb.modular.controller;

import com.baomidou.mybatisplus.core.metadata.IPage;
import com.lplb.core.pojo.response.ResponseData;
import com.lplb.core.pojo.response.SuccessResponseData;
import com.lplb.modular.model.entity.DRnaTsScore;
import com.lplb.modular.model.query.DRnaTsScoreQuery;
import com.lplb.modular.model.vo.DRnaTsScoreVo;
import com.lplb.modular.service.DRnaTsScoreService;
import io.swagger.annotations.Api;
import io.swagger.annotations.ApiOperation;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;
import org.springframework.web.multipart.MultipartFile;

import javax.annotation.Resource;

/**
 * @Author（作者）： BealHe
 * @Date（日期）： 2023-07-27 18:35:30
 * @Description（描述）：D RNA TS score(DRnaTsScore)表控制层
 */
@RestController
@RequestMapping("/web/dRnaTsScore")
@Api(value = "DRnaTsScoreController", tags = "D RNA TS score(DRnaTsScore)")
public class DRnaTsScoreController {
    @Resource
    private DRnaTsScoreService dRnaTsScoreService;

    @PostMapping(value = "/importExcel")
    @ApiOperation(value = "Excel导入")
    public ResponseData<Boolean> importExcel(MultipartFile file) {
        Boolean result = dRnaTsScoreService.importExcel(file);
        return SuccessResponseData.success(result);
    }

    @GetMapping(value = "/page")
    @ApiOperation(value = "数据列表查询")
    public ResponseData<IPage<DRnaTsScoreVo>> page(DRnaTsScoreQuery query) {
        IPage<DRnaTsScoreVo> result = dRnaTsScoreService.pageList(query);
        return ResponseData.success(result);
    }
}

