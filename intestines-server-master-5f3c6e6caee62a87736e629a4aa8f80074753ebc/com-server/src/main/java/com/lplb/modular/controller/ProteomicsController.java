 ...
 * @Description（描述）：Proteomics（蛋白质组学）(Proteomics)表控制层
 */
@RestController
@RequestMapping("/web/proteomics")
@Api(value = "ProteomicsController", tags = "Proteomics（蛋白质组学）(Proteomics)")
public class ProteomicsController {
    ...
    public ResponseData<Boolean> importExcel(MultipartFile file) {
        Boolean result = proteomicsService.importExcel(file);
        return SuccessResponseData.success(result);
    }

    @GetMapping(value = "/page")
    @ApiOperation(value = "数据列表查询")
    public ResponseData<IPage<Proteomics>> page(ProteomicsQuery query) {
        IPage<Proteomics> result = proteomicsService.pageList(query);
    ...
    public void exportExcel(HttpServletResponse response, ProteomicsQuery query) {
        proteomicsService.exportExcel(response, query);
    }
}
