...
import com.lplb.modular.mapper.SeMatsJcecMapper;
 ...
 * @Description（描述）：SE.MATS.JCEC(SeMatsJcec)表服务实现类
 */
@Service
@Transactional
public class SeMatsJcecServiceImpl extends ServiceImpl<SeMatsJcecMapper, SeMatsJcec> implements SeMatsJcecService {

    private final String tableName = "in_se_mats_jcec";
    ...
    private final String fileName = "SE_MATS_JCEC";
    ...
    public Boolean importExcel(MultipartFile file) {
        ...
        try {
            ...
            // 获取列表
            List<SeMatsJcec> list = reader.readAll(SeMatsJcec.class);
...
//                                item.getId());
     ...
     */
    @Override
    public IPage<SeMatsJcec> pageList(SeMatsJcecQuery query) {
        // 分页
        Page<SeMatsJcec> page;
        ...
        // 查询条件
        LambdaQueryWrapper<SeMatsJcec> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(SeMatsJcec::getDataAccessId, query.getDataAccessId());
        wrapper.eq(SeMatsJcec::getFileNo, query.getFile());
        if (ObjectUtils.isNotEmpty(query.getKeywords())) {
            wrapper.like(SeMatsJcec::getGeneName, query.getKeywords());
        }
        if (ObjectUtils.isNotEmpty(query.getEnsembleId())) {
            wrapper.eq(SeMatsJcec::getEnsembleId, query.getEnsembleId());
        }
        ...
        if (ObjectUtils.isNotEmpty(query.getOrderBy())) {
            ...
            if ("exonStart0Base".equals(order[0])) {
                colum = "exon_start_0_base";
    ...
    public void exportExcel(HttpServletResponse response, SeMatsJcecQuery query) {
        ...
        // 查询数据集合
        List<SeMatsJcec> list;
        int count = this.count(wrapper);
        if (count > CommonConstant.MAX_EXPORT_COUNT) {
            Page<SeMatsJcec> page = new Page<>(query.getExportCurrent(), query.getExportSize());
        ...
        // 导出数据集合
        List<SeMatsJcecExport> exports = BeanConvertUtil.listConvert(list, SeMatsJcecExport.class);

        try {
            response.setContentType("text/csv");
            response.setCharacterEncoding("utf-8");
            String name = "Omics_Transcriptomics_Alternative Splicing_Data Access_SE.MATS.JCEC_" + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss"));
            String fileName = URLEncoder.encode(name, "UTF-8").replaceAll("\\+", "%20");
            response.setHeader("Content-disposition", "attachment;filename*=utf-8''" + fileName + ".csv");
            EasyExcel.write(response.getOutputStream(), SeMatsJcecExport.class)
    ...
    private ExcelReader readHeaderAlias(ExcelReader reader) {
        ...
        reader.addHeaderAlias("exonStart_0base", "exonStart0Base");
        reader.addHeaderAlias("exonEnd", "exonEnd");
    ...
    private ExcelWriter writerHeaderAlias(ExcelWriter writer) {
        ...
        writer.addHeaderAlias("exonStart0Base", "exonStart_0base");
        writer.addHeaderAlias("exonEnd", "exonEnd");
