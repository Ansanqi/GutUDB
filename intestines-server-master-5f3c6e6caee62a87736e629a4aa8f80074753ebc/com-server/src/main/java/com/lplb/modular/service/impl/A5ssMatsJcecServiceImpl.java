...
import com.lplb.modular.mapper.A5ssMatsJcecMapper;
import com.lplb.modular.model.entity.A5ssMatsJcec;
import com.lplb.modular.model.export.A5ssMatsJcecExport;
 ...
 * @Description（描述）：A5SS.MATS.JCEC(A5ssMatsJcec)表服务实现类
 */
@Service
@Transactional
public class A5ssMatsJcecServiceImpl extends ServiceImpl<A5ssMatsJcecMapper, A5ssMatsJcec> implements A5ssMatsJcecService {

    private final String tableName = "in_a5ss_mats_jcec";
    ...
    public Boolean importExcel(MultipartFile file) {
        ...
        try {
            ...
            // 获取列表
            List<A5ssMatsJcec> list = reader.readAll(A5ssMatsJcec.class);
...
//                                item.getId());
     ...
     */
    @Override
    public IPage<A5ssMatsJcec> pageList(A5ssMatsJcecQuery query) {
        // 分页
        Page<A5ssMatsJcec> page;
        ...
        // 查询条件
        LambdaQueryWrapper<A5ssMatsJcec> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(A5ssMatsJcec::getDataAccessId, query.getDataAccessId());
        wrapper.eq(A5ssMatsJcec::getFileNo, query.getFile());
//        if (ObjectUtils.isNotEmpty(query.getSearch())) {
//            wrapper.like(A5ssMatsJcec::getEnsembleId, query.getSearch())
//                    .or()
//                    .like(A5ssMatsJcec::getGeneName, query.getSearch())
//                    .or()
//                    .like(A5ssMatsJcec::getChr, query.getSearch());
//        }
        if (ObjectUtils.isNotEmpty(query.getKeywords())) {
            wrapper.like(A5ssMatsJcec::getGeneName, query.getKeywords());
        }
        if (ObjectUtils.isNotEmpty(query.getEnsembleId())) {
            wrapper.eq(A5ssMatsJcec::getEnsembleId, query.getEnsembleId());
        }
    ...
    public void exportExcel(HttpServletResponse response, A5ssMatsJcecQuery query) {
        ...
        // 查询数据集合
        List<A5ssMatsJcec> list;
        int count = this.count(wrapper);
        if (count > CommonConstant.MAX_EXPORT_COUNT) {
            Page<A5ssMatsJcec> page = new Page<>(query.getExportCurrent(), query.getExportSize());
        ...
        // 导出数据集合
        List<A5ssMatsJcecExport> exports = BeanConvertUtil.listConvert(list, A5ssMatsJcecExport.class);

        try {
            response.setContentType("text/csv");
            response.setCharacterEncoding("utf-8");
            String name = "Omics_Transcriptomics_Alternative Splicing_Data Access_A5SS.MATS.JCEC_" + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss"));
            String fileName = URLEncoder.encode(name, "UTF-8").replaceAll("\\+", "%20");
            response.setHeader("Content-disposition", "attachment;filename*=utf-8''" + fileName + ".csv");
            EasyExcel.write(response.getOutputStream(), A5ssMatsJcecExport.class)
