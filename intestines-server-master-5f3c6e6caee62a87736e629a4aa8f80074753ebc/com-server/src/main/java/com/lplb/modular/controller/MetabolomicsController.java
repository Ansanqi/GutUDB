 ...
 * @Description（描述）：Metabolomics（代谢组学）(Metabolomics)表控制层
 */
@RestController
@RequestMapping("/web/metabolomics")
@Api(value = "MetabolomicsController", tags = "Metabolomics（代谢组学）(Metabolomics)")
public class MetabolomicsController {
    @Resource
    ...
    public ResponseData<Boolean> importExcel(MultipartFile file) {
        Boolean result = metabolomicsService.importExcel(file);
        return SuccessResponseData.success(result);
    }

    @GetMapping(value = "/page")
    @ApiOperation(value = "数据列表查询")
    public ResponseData<IPage<Metabolomics>> page(MetabolomicsQuery query) {
        IPage<Metabolomics> result = metabolomicsService.pageList(query);
    ...
    public void exportExcel(HttpServletResponse response, MetabolomicsQuery query) {
        metabolomicsService.exportExcel(response, query);
    }
}
